...
# Set of sites in 1D with 1/cm scales
# in these units t=1 is 3.33333e-11 s
#   i.e. t=1 is 1cm of propogation
...
class simulator():
    ...
    def __init__(self, 
        ...
        # Should be fixed by _extend_hamiltonian as it's value depends on subclass
    ...
    def _gen_initial_state(self, initial_state=NB_INITIAL_STATE):
        ...
        if type(initial_state) == int:
            ...
            assert False, " this functionality has not beed added yet"
            return initial_state
        else:
            assert False, "input state not understood"
            return None
...
if __name__ == '__main__':
    test = waveguide_reversible_sink(sink_sites=[[2,5],[5, 10]], 
                                      couplings = LS_COUPLINGS, 
                                      seed=10, 
                                      delta_beta=100)
    test.update_hamiltonian(seed = int(time.time()))
    out = test.simulate_waveguide(plot_end=True)
    test.quick_summary()
